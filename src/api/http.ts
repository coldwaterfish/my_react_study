...
const Axios = (conf: IConf = defaultConf) => {
  /**
   * 1. 初始化一个service
  ...
  service.interceptors.request.use(
    (config: AxiosRequestConfig) => {
      ...
      let token = localStorage.getItem(TOKEN_KEY); // 获取本地token
      if (token) {
    ...
    (error: { config: { headers: { isLoading: boolean; }; }; data: { msg?: any; }; }) => {
  ...
  service.interceptors.response.use(
    ...
    async (error: { config: { headers: any; }; message: string; data: { msg?: any; }; }) => {
      if (error.config.headers!.isLoading !== false) {
        hideLoading(loadingCount);
      }
